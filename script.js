...
let chapterIndices = [];
let chapterIndexByName = new Map();
const chapterNames = [
    "1 Nephi 1",
    "1 Nephi 2",
    "1 Nephi 3",
...
function parseScripture(text) {
    ...
    for (let line of lines) {
        // Chapter header
        if (line.startsWith("#") || chapterNameSet.has(line)) {
            if (currentVerse) {
                items.push(currentVerse);
                currentVerse = "";
            }
            const currentChapter = line.startsWith("#") ? line.substring(1).trim() : line;
...
function updateChapterSelect() {
    ...
    if (chapterIndex !== null) {
        const chapterName = items[chapterIndex].text;
        if (chapterNameSet.has(chapterName)) {
